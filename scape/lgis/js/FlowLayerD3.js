define(["dojo/_base/declare"], function (declare) {
     ...
     */
    function createSettings(topo,colors) {
        debugger;
        var isFF = /firefox/i.test(navigator.userAgent);
        //var topo1 = { "bbox": [138, 35, 140, 36], };
        ...
        var bounds = topo.bounds;
        var styles = [];
        var settings = {
            projection: projection,
            displayBounds: bounds,
            particleCount: Math.round(bounds.height / 1.01),
    ...
    function clearCanvas(canvas) {
        if (canvas) {
            //canvas.getContext("2d").clearRect(0, 0, canvas.width, canvas.height).remove();
            canvas.remove();
        }
    ...
    function drawOverlay(stations, data, settings, masks) {
        ...
        var n = view.width / 5;
        for (var i = n; i >= 0; i--) {
            g.fillStyle = asRainbowColorStyle((1 - (i / n)), 0.2);
    ...
    function build(setting, stations, data) {

        var renderTask = render(setting, null);
        var stationsTask = prepareStations(stations);
        var dataTask = prepareData(data);
        var plotStationsTask = plotStations(stationsTask, data, null);
        var overlayTask = when.all([stationsTask, dataTask, setting, renderTask]).then(apply(drawOverlay));
        var fieldTask = when.all([stationsTask, dataTask, setting, renderTask]).then(apply(interpolateField));
        var animateTask = when.all([setting, fieldTask]).then(apply(animate));
        var postInitTask = when.all([setting, fieldTask, null]).then(apply(postInit));
        return;
        var dataTask = loadJson(displayData.samples).then(prepareData);
        var plotStationsTask = when.all([dataTask]).then(apply(plotStations));
    ...
    return declare("FlowLayerD3", null, {
        setting: null,
        mapView: null,
        constructor: function (box) {

            this.mapView = box.view;
            ...
            box.bounds = { "x": 0, "y": 0, "width": this.mapView.width, "height": this.mapView.height };
            this.setting = createSettings(box, box.colors);
        },
        build: function (data) {
            if (data.render) {
                displayData.recipe = RECIPES[data.render];
                displayData.type = data.render;
                var canvas = document.createElement('canvas');
            ...
            build(this.setting, data.stations, data.data);
        },
        stop: function () {
            stop(this.setting);
        }

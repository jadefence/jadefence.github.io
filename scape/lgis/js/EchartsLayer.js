define([
  ...
  return declare("EchartsLayer", null, {
    ...
    _mapOffset: [0, 0],
    _mapEvent:null,
    constructor: function (view, ec) {
      this._view = view;
      this._map = view.map;
      var div = this._echartsContainer = document.createElement('div');
      div.className = "lgisEchartsLayer";
    ...
    _init: function(map, ec) {
      ...
      self.geoCoord2Pixel = function (geoCoord) {
          ...
          if (this._view.spatialReference.wkid == "102100") {
              var xy = lonlat2mercator({ x: geoCoord[0], y: geoCoord[1] });
              point = new Point({
                  x: xy.x,
                  y: xy.y,
              ...
              point = new Point(geoCoord[0], geoCoord[1]);
          }
        
        //var pos = self._map.toScreen(point);
        var pos = this._view.toScreen(point);
        return [pos.x, pos.y];
      };
      ...
      self.pixel2GeoCoord = function(pixel) {
          ...
          var point = this._view.toMap(new ScreenPoint(pixel[0], pixel[1]));
      ...
      self._bindEvent = function() {
        ...
        //});
          self._mapEvent = this._view.watch('stationary', function (newValue) {
            if (newValue) {
